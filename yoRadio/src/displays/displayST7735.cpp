...
//  dsp.clearClock();
  ...
  uint8_t real_size = qrcode.size*m;
  uint8_t x0 = (width()-real_size)/2;
  uint8_t y0 = (height()-real_size)/2;
  u_int16_t color = inv?BOOT_TXT_COLOR:0;
  startWrite();
  writeFillRect(0,0,width(),height(),inv?0:BOOT_TXT_COLOR);
  for(uint8_t y=0; y<qrcode.size; y++ ){
    for(uint8_t x=0; x<qrcode.size; x++){
      if(qrcode_getModule(&qrcode, x, y))writeFillRect(x0+x*m,y0+y*m,m,m,color);
    }
  }
  endWrite();
}
