...
bool NetServer::begin(bool quiet) {
  ...
  webserver.on(TMP_PATH, HTTP_GET, handleHTTPArgs);
...
void NetServer::processQueue(){
  ...
  if(xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS)){
    ...
    switch (request.type) {
      ...
      case GETMODE:       sprintf (wsbuf, "{\"pmode\":\"%s\",\"fav_cnt\":%d}", network.status == CONNECTED ? "player" : "ap",config.get_fav_cnt()); break;
      case GETINDEX:      {
...
bool NetServer::importPlaylist() {
  ...
  if (config.parseCSV(linePl, sName, sUrl, sOvol, fav)) {
    tempfile.close();
