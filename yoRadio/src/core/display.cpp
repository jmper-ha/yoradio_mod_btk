...
void Display::_setReturnBackTicker(uint8_t time_s){
  _returnTicker.detach();
  _returnTicker.once(time_s, returnBackPage);
}
...
void Display::_swichMode(displayMode_e newmode) {
  ...
  #endif
  if(_mode == QR && newmode == QR){
    if(network.status == CONNECTED) returnPlayer();
    else setBootPage();
    _returnTicker.detach();
    return;
  }
  if (newmode == _mode || (network.status != CONNECTED && network.status != SDREADY) && !(newmode == QR && network.status == SOFT_AP)) return;
  _mode_back = _mode;
  _mode = newmode;
  dsp.setScrollId(NULL);
  if (newmode == PLAYER) {
...
void Display::drawQRcode() {
  ...
  if(network.status == CONNECTED){
    sprintf(qr_txt,"http://%s",WiFi.localIP().toString().c_str());
    _setReturnTicker(15);
  }else{
    if(WiFi.softAPgetStationNum()){
      sprintf(qr_txt,"http://%s",WiFi.softAPIP().toString().c_str());
    } else {
      sprintf(qr_txt,"WIFI:T:WPA2;S:%s;P:%s;;",apSsid,apPassword);
      inv = false;
    }
    _setReturnBackTicker(15);
  }
  _pager.setPage(pages[PG_SCREENSAVER]);
  dsp.drawQR(qr_txt, inv);
}
...
void upd_charge_cb(){
  display.putRequest(CHARGE, false);
}

void Display::_upd_charge(){
  char batt[4];
  switch (charge.phase){
    case 0:{
      if(charge.flip) strlcpy(batt, "\027\030\032", 4);
      else strlcpy(batt, "\027\030\033", 4);
    } break;
    case 1:{
      if(charge.flip) strlcpy(batt, "\027\030\033", 4);
      else strlcpy(batt, "\027\031\033", 4);
    } break;
    case 2:{
      if(charge.flip) strlcpy(batt, "\027\031\033", 4);
      else strlcpy(batt, "\013\031\033", 4);
    } break;
    case 3:{
      if(charge.flip) strlcpy(batt, "\013\031\033", 4);
      else strlcpy(batt, "\040\040\040", 4);
    } break;
    default:
      break;
  }
...
void Display::_updBatt(uint16_t data){
  if(!_bat) return;
  char batt[4];
  uint8_t pers = data;
  bool is_charge = data>>8;
  if(pers == 100) is_charge = false;
  if(pers>75) charge.phase = 0;
  else if(pers>50 && pers<=75) charge.phase = 1;
  else if(pers>25 && pers<=50) charge.phase = 2;
  else charge.phase = 3;
  _bat->setFgColor(charge.phase == 3?config.theme.batt_low:config.theme.batt);
  _battxt->setText(pers, "%d%%");
  if(is_charge){
    if(!charge.is_charge){
      _chargeTicker.attach_ms(500, upd_charge_cb);
      charge.flip = false;
    }
  } else {
    switch (charge.phase){
      case 0: strlcpy(batt, "\027\030\032", 4);break; //100-75
      case 1: strlcpy(batt, "\027\030\033", 4);break; //75-50
      case 2: strlcpy(batt, "\027\031\033", 4);break; //50-25
      case 3: strlcpy(batt, "\013\031\033", 4);break; //25-0
    default: break;
    }
    _bat->setText(batt);
    if(charge.is_charge) _chargeTicker.detach();
  }
  charge.is_charge = is_charge;
}
...
/*
void Display::uart_sent_dsp(bool stat){
  display.putRequest(SHOWBT, stat);
}*/
//============================================================================================================================
#else // !DUMMYDISPLAY
//============================================================================================================================
