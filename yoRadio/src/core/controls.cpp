...
#if ISPUSHBUTTONS
int btn[] = {BTN_LEFT,BTN_CENTER,BTN_RIGHT,ENC_BTNB,BTN_UP,BTN_DOWN,ENC2_BTNB,BTN_MODE,BTN_QR,BTN_F1,BTN_F2,BTN_F3,BTN_F4,BTN_F5,BTN_F6};
//#define BTN_FAV_SHIFT 9
constexpr uint8_t nrOfButtons = sizeof(btn) / sizeof(btn[0]);
#include "../OneButton/OneButton.h"
OneButton button[nrOfButtons];
...
void initControls() {
  ...
  for (int i = 0; i < nrOfButtons; i++){
    bool pullup = (i==ENC_BTNB)?ENC_INTERNALPULLUP:((i==ENC2_BTNB)?ENC2_INTERNALPULLUP:BTN_INTERNALPULLUP);
    button[i] = OneButton(btn[i], true, pullup);
    if(btn[i] == 255) continue;
...
#       endif
    ...
    case EVT_F6: {
      lpId = id;
      lpd = LONG_PRESS_DURATION;
      Serial.printf("******************  Favorite button id %i LongPress\n",id-EVT_F1);
      break;
    }
    default: break;
  }
...
void onBtnDuringLongPress(int id) {
  ...
  if (checklpdelay(BTN_LONGPRESS_LOOP_DELAY, lpdelay)) {
    switch ((controlEvt_e)id) {
      ...
      case EVT_F6:{
        if(--lpd==1){
          uint8_t res = config.addFavorite(id-EVT_F1);
          if(!res){
            Serial.printf("******************  Favorite button id %i Save stantion\n",id-EVT_F1);
            start_buzzer();
            netserver.requestOnChange(GETFAVUPD, 0);
          }else if(res>0){
            display.putRequest(SHOWFAVWARN, res);
          }
        }
        if(lpd==0){
          lpId = -1;
...
void onBtnClick(int id) {
  ...
  bool passQRSAP = network.status==SOFT_AP && (controlEvt_e)id == EVT_QR;
  controlEvt_e btnid = static_cast<controlEvt_e>(id);
  pm.on_btn_click(btnid);
  if (network.status != CONNECTED && network.status!=SDREADY && (controlEvt_e)id!=EVT_BTNMODE && !passBnCenter && !passQRSAP) return;
  switch (btnid) {
    case EVT_BTNLEFT: {
        controlsEvent(false);
        break;
    ...
    case EVT_F5:
    case EVT_F6: {
      Serial.printf("******************  Favorite button id %i click\n",id-EVT_F1);
      config.playFavorite(id-EVT_F1);
      break;
    }
    case EVT_QR:
      Serial.printf("#######  QR button id %i click\n",id);
      display.putRequest(NEWMODE, QR);
      break;
    default: break;
  }
}
...
void start_buzzer(void){
  #if BUZZER!=255
    ESP_ERROR_CHECK(ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_0, 128));// 8_BIT/2 = 50%
