...
void Config::loadTheme(){
  ...
  theme.bt            = color565(COLOR_BLUETOOTH);
  theme.batt          = color565(COLOR_BATTERY);
  theme.batt_low      = color565(COLOR_BATTERY_LOW);
...
int8_t Config::addFavorite(uint16_t fav_id){
  uint8_t res = 0;
  uint16_t old_line;
  for(int i=0;i<FAVBTNCNT;i++)
    if(favorite[i]==store.lastStation) res = i+1;
  if(res) return res;
  File playlist = SPIFFS.open(PLAYLIST_PATH, "r");
  ...
  int sOvol,fav, i = 1;
  while (playlist.available()) {
    int bytesRead = playlist.readBytesUntil('\n', linePl, sizeof(linePl)-1);
    if(bytesRead>0){
      linePl[bytesRead] = 0;
      if(linePl[bytesRead-1]=='\r') linePl[bytesRead-1]=0;
      if (parseCSV(linePl, sName, sUrl, sOvol,fav)){
        if(fav_id==fav-1){ 
          fav=0;
          old_line = i;
        }
        if(store.lastStation == i){
          favorite[fav_id] = i;
          fav = fav_id+1;
        }
        snprintf(linePl, sizeof(linePl)-1, "%s\t%s\t%d\t%d\n",sName,sUrl,sOvol,fav);
        tmp.print(linePl);
        i++;
      }
    }
  ...
  snprintf(upd_fav,sizeof(upd_fav),"{\"uf\":[{\"l\":%d,\"v\":0},{\"l\":%d,\"v\":%d}]}",old_line,config.store.lastStation,fav_id+1);
  return 0;
}

char* Config::updFavLine(){
  return Config::upd_fav;
}

void Config::playFavorite(uint8_t id){
  uint16_t fav = favorite[id];
  if(fav) player.sendCommand({PR_PLAY, fav});
}

uint8_t Config::get_fav_cnt(){
  return FAVBTNCNT;
}
...
bool Config::parseCSV(const char* line, char* name, char* url, int &ovol, int &f) {
  char *tmpe;
